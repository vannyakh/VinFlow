from django.core.management.base import BaseCommand
from panel.models import SystemSetting

class Command(BaseCommand):
    help = 'Initialize default system settings'

    def handle(self, *args, **options):
        default_settings = [
            # General Settings
            {
                'key': 'site_name',
                'label': 'Site Name',
                'setting_type': 'text',
                'group': 'general',
                'default_value': 'VinFlow SMM Panel',
                'description': 'The name of your SMM panel',
                'order': 1,
            },
            {
                'key': 'site_logo',
                'label': 'Site Logo',
                'setting_type': 'image',
                'group': 'general',
                'default_value': '',
                'description': 'Site logo image (upload or enter URL/path)',
                'order': 2,
            },
            {
                'key': 'site_description',
                'label': 'Site Description',
                'setting_type': 'textarea',
                'group': 'general',
                'default_value': 'Professional SMM Panel for Social Media Marketing',
                'description': 'Site meta description for SEO',
                'order': 3,
            },
            {
                'key': 'site_url',
                'label': 'Site URL',
                'setting_type': 'url',
                'group': 'general',
                'default_value': '',
                'description': 'Full site URL (e.g., https://example.com) for canonical links and Open Graph',
                'order': 4,
            },
            {
                'key': 'site_email',
                'label': 'Site Email',
                'setting_type': 'email',
                'group': 'general',
                'default_value': 'admin@vinflow.com',
                'description': 'Main contact email address',
                'order': 5,
            },
            {
                'key': 'site_keywords',
                'label': 'Meta Keywords',
                'setting_type': 'text',
                'group': 'seo',
                'default_value': 'SMM Panel, Social Media Marketing, Instagram, Facebook, Twitter, YouTube, TikTok',
                'description': 'Comma-separated keywords for SEO meta tags',
                'order': 1,
            },
            {
                'key': 'site_author',
                'label': 'Site Author',
                'setting_type': 'text',
                'group': 'seo',
                'default_value': '',
                'description': 'Author name for meta tags',
                'order': 2,
            },
            {
                'key': 'og_image',
                'label': 'Open Graph Image',
                'setting_type': 'image',
                'group': 'seo',
                'default_value': '',
                'description': 'Image for social media sharing (1200x630px recommended)',
                'order': 3,
            },
            {
                'key': 'og_type',
                'label': 'Open Graph Type',
                'setting_type': 'text',
                'group': 'seo',
                'default_value': 'website',
                'description': 'Open Graph type (website, article, etc.)',
                'order': 4,
            },
            {
                'key': 'twitter_card',
                'label': 'Twitter Card Type',
                'setting_type': 'text',
                'group': 'seo',
                'default_value': 'summary_large_image',
                'description': 'Twitter Card type (summary, summary_large_image, etc.)',
                'order': 5,
            },
            {
                'key': 'twitter_site',
                'label': 'Twitter Site Handle',
                'setting_type': 'text',
                'group': 'seo',
                'default_value': '',
                'description': 'Twitter username (without @)',
                'order': 6,
            },
            {
                'key': 'google_analytics_id',
                'label': 'Google Analytics ID',
                'setting_type': 'text',
                'group': 'seo',
                'default_value': '',
                'description': 'Google Analytics tracking ID (e.g., G-XXXXXXXXXX or UA-XXXXXXXXX-X)',
                'order': 7,
            },
            {
                'key': 'google_search_console',
                'label': 'Google Search Console Verification',
                'setting_type': 'text',
                'group': 'seo',
                'default_value': '',
                'description': 'Google Search Console verification code',
                'order': 8,
            },
            {
                'key': 'robots_index',
                'label': 'Allow Search Engine Indexing',
                'setting_type': 'boolean',
                'group': 'seo',
                'default_value': 'true',
                'description': 'Allow search engines to index the site',
                'order': 9,
            },
            {
                'key': 'robots_follow',
                'label': 'Allow Search Engine Following',
                'setting_type': 'boolean',
                'group': 'seo',
                'default_value': 'true',
                'description': 'Allow search engines to follow links',
                'order': 10,
            },
            {
                'key': 'favicon',
                'label': 'Favicon',
                'setting_type': 'image',
                'group': 'seo',
                'default_value': '',
                'description': 'Favicon image (32x32px or 16x16px recommended)',
                'order': 11,
            },
            {
                'key': 'site_language',
                'label': 'Site Language',
                'setting_type': 'text',
                'group': 'seo',
                'default_value': 'en',
                'description': 'Primary site language code (ISO 639-1)',
                'order': 12,
            },
            {
                'key': 'site_region',
                'label': 'Site Region',
                'setting_type': 'text',
                'group': 'seo',
                'default_value': '',
                'description': 'Site region code (ISO 3166-1 alpha-2)',
                'order': 13,
            },
            {
                'key': 'min_deposit',
                'label': 'Minimum Deposit',
                'setting_type': 'number',
                'group': 'payment',
                'default_value': '5.00',
                'description': 'Minimum deposit amount',
                'order': 1,
            },
            {
                'key': 'max_deposit',
                'label': 'Maximum Deposit',
                'setting_type': 'number',
                'group': 'payment',
                'default_value': '10000.00',
                'description': 'Maximum deposit amount',
                'order': 2,
            },
            {
                'key': 'currency',
                'label': 'Currency',
                'setting_type': 'text',
                'group': 'payment',
                'default_value': 'USD',
                'description': 'Default currency code',
                'order': 3,
            },
            {
                'key': 'currency_symbol',
                'label': 'Currency Symbol',
                'setting_type': 'text',
                'group': 'payment',
                'default_value': '$',
                'description': 'Currency symbol',
                'order': 4,
            },
            # Payment Gateway Settings
            {
                'key': 'payment_paypal_enabled',
                'label': 'Enable PayPal',
                'setting_type': 'boolean',
                'group': 'payment',
                'default_value': 'true',
                'description': 'Enable PayPal payment gateway',
                'order': 10,
            },
            {
                'key': 'payment_stripe_enabled',
                'label': 'Enable Stripe',
                'setting_type': 'boolean',
                'group': 'payment',
                'default_value': 'true',
                'description': 'Enable Stripe payment gateway',
                'order': 11,
            },
            {
                'key': 'payment_khqr_enabled',
                'label': 'Enable KHQR Bakong',
                'setting_type': 'boolean',
                'group': 'payment',
                'default_value': 'false',
                'description': 'Enable KHQR Bakong payment gateway',
                'order': 12,
            },
            {
                'key': 'enable_registration',
                'label': 'Enable Registration',
                'setting_type': 'boolean',
                'group': 'security',
                'default_value': 'true',
                'description': 'Allow new user registration',
                'order': 1,
            },
            {
                'key': 'require_email_verification',
                'label': 'Require Email Verification',
                'setting_type': 'boolean',
                'group': 'security',
                'default_value': 'false',
                'description': 'Require email verification for new accounts',
                'order': 2,
            },
            {
                'key': 'maintenance_mode',
                'label': 'Maintenance Mode',
                'setting_type': 'boolean',
                'group': 'maintenance',
                'default_value': 'false',
                'description': 'Enable maintenance mode',
                'order': 1,
            },
            {
                'key': 'maintenance_message',
                'label': 'Maintenance Message',
                'setting_type': 'textarea',
                'group': 'maintenance',
                'default_value': 'We are currently performing maintenance. Please check back soon.',
                'description': 'Message to display during maintenance',
                'order': 2,
            },
            {
                'key': 'api_enabled',
                'label': 'API Enabled',
                'setting_type': 'boolean',
                'group': 'api',
                'default_value': 'true',
                'description': 'Enable API access',
                'is_public': True,
                'order': 1,
            },
            {
                'key': 'api_rate_limit',
                'label': 'API Rate Limit',
                'setting_type': 'number',
                'group': 'api',
                'default_value': '100',
                'description': 'API requests per minute',
                'order': 2,
            },
            # SMM Service Supplier Settings
            {
                'key': 'supplier_jap_url',
                'label': 'JAP API URL',
                'setting_type': 'url',
                'group': 'api',
                'default_value': 'https://justanotherpanel.com/api/v2',
                'description': 'JustAnotherPanel API endpoint URL',
                'order': 10,
            },
            {
                'key': 'supplier_jap_key',
                'label': 'JAP API Key',
                'setting_type': 'text',
                'group': 'api',
                'default_value': '',
                'description': 'JustAnotherPanel API key',
                'is_encrypted': True,
                'order': 11,
            },
            {
                'key': 'supplier_timeout',
                'label': 'Supplier API Timeout',
                'setting_type': 'number',
                'group': 'api',
                'default_value': '30',
                'description': 'Timeout in seconds for supplier API requests',
                'order': 16,
            },
            {
                'key': 'supplier_retry_attempts',
                'label': 'Supplier Retry Attempts',
                'setting_type': 'number',
                'group': 'api',
                'default_value': '3',
                'description': 'Number of retry attempts for failed supplier API calls',
                'order': 17,
            },
            {
                'key': 'auto_sync_order_status',
                'label': 'Auto Sync Order Status',
                'setting_type': 'boolean',
                'group': 'api',
                'default_value': 'true',
                'description': 'Automatically sync order status from suppliers',
                'order': 18,
            },
            {
                'key': 'sync_interval_minutes',
                'label': 'Order Status Sync Interval',
                'setting_type': 'number',
                'group': 'api',
                'default_value': '5',
                'description': 'Interval in minutes for syncing order status',
                'order': 19,
            },
            # Marketing & Promotion Settings
            {
                'key': 'marketing_enabled',
                'label': 'Enable Marketing System',
                'setting_type': 'boolean',
                'group': 'general',
                'default_value': 'true',
                'description': 'Enable marketing promotions and campaigns',
                'order': 20,
            },
            {
                'key': 'promotions_auto_display',
                'label': 'Auto Display Promotions',
                'setting_type': 'boolean',
                'group': 'general',
                'default_value': 'true',
                'description': 'Automatically display active promotions to users',
                'order': 21,
            },
            {
                'key': 'promotion_popup_delay',
                'label': 'Promotion Popup Delay',
                'setting_type': 'number',
                'group': 'general',
                'default_value': '3',
                'description': 'Delay in seconds before showing promotion popup',
                'order': 22,
            },
            {
                'key': 'promotion_max_per_page',
                'label': 'Max Promotions Per Page',
                'setting_type': 'number',
                'group': 'general',
                'default_value': '3',
                'description': 'Maximum number of promotions to display on a single page',
                'order': 23,
            },
            {
                'key': 'first_order_discount',
                'label': 'First Order Discount',
                'setting_type': 'number',
                'group': 'general',
                'default_value': '10',
                'description': 'Discount percentage for first-time orders',
                'order': 24,
            },
            {
                'key': 'referral_bonus_amount',
                'label': 'Referral Bonus Amount',
                'setting_type': 'number',
                'group': 'general',
                'default_value': '5.00',
                'description': 'Bonus amount for successful referrals',
                'order': 25,
            },
            {
                'key': 'new_user_welcome_bonus',
                'label': 'New User Welcome Bonus',
                'setting_type': 'number',
                'group': 'general',
                'default_value': '0.00',
                'description': 'Welcome bonus amount for new user registrations',
                'order': 26,
            },
            {
                'key': 'flash_sale_notification',
                'label': 'Flash Sale Notifications',
                'setting_type': 'boolean',
                'group': 'general',
                'default_value': 'true',
                'description': 'Send notifications for flash sales',
                'order': 27,
            },
            {
                'key': 'promotional_email_enabled',
                'label': 'Promotional Emails',
                'setting_type': 'boolean',
                'group': 'email',
                'default_value': 'true',
                'description': 'Allow sending promotional emails to users',
                'order': 20,
            },
            {
                'key': 'promotional_email_frequency',
                'label': 'Promotional Email Frequency',
                'setting_type': 'number',
                'group': 'email',
                'default_value': '7',
                'description': 'Minimum days between promotional emails to same user',
                'order': 21,
            },
        ]

        created_count = 0
        updated_count = 0

        for setting_data in default_settings:
            key = setting_data.pop('key')
            setting, created = SystemSetting.objects.get_or_create(
                key=key,
                defaults=setting_data
            )
            if created:
                created_count += 1
                self.stdout.write(self.style.SUCCESS(f'Created setting: {key}'))
            else:
                # Update existing settings with new defaults if they don't have values
                if not setting.value and setting_data.get('default_value'):
                    setting.value = setting_data['default_value']
                    setting.save()
                    updated_count += 1
                    self.stdout.write(self.style.WARNING(f'Updated setting: {key}'))

        self.stdout.write(
            self.style.SUCCESS(
                f'\nSuccessfully initialized settings: {created_count} created, {updated_count} updated'
            )
        )

